...
def _md_to_html(text: str) -> str:
    ...
    Also normalises literal ``\\n`` escape sequences (produced by shell
    ``copaw channels send --text "..."`` where the agent writes ``\\n`` as
    text rather than a real newline) into real newlines so that markdown-it
...
class MatrixChannel(BaseChannel):
    ...
    async def send(
        ...
        if text.strip().startswith("NO_REPLY"):
            logger.info(
